...
function buildClinicalTaskStory(task, patient) {
  const reason = String(task.reason || "");
  const parts = [];

  const medMatch = reason.match(/Missed medication:\s*([^,]+(?:,\s*[^,]+)*)/i);
  if (medMatch) {
    const medsText = medMatch[1].trim();
    const meds = medsText.split(",").map(s => s.trim()).filter(Boolean);

    meds.forEach(med => {
      const lower = med.toLowerCase();
      if (lower.includes("lasix") || lower.includes("furosemide") || lower.includes("bumex") || lower.includes("bumetanide") || lower.includes("torsemide")) {
  ...
  if (/Severe shortness of breath/i.test(reason)) {
    parts.push(`Respiratory symptoms → worsening heart failure`);
  } else if (/Moderate shortness of breath/i.test(reason) || /Mild shortness of breath/i.test(reason)) {
    parts.push(`Respiratory symptoms → worsening heart failure`);
  }
...
function renderTasks(tasks) {
  ...
  byId("task-list").innerHTML = tasks.map(task => {
    ...
    const clinical = buildClinicalTaskStory(task, patient);
